...
for i =  1:N
    ...
    MSE_dct_list(i)  = mean((X - dct_Yd).^2, 'all');
    SURE_dct_list(i) = mean(SURE_temp_list,'all');
    PSNR_dct_list(i) = PSNR(X,dct_Yd,1.0);
    ...
    MSE_dft_list(i)  = mean((X - dft_Yd).^2, 'all');
    SURE_dft_list(i) = mean(SURE_temp_list,'all');
    PSNR_dft_list(i) = PSNR(X,dft_Yd,1.0);
    ...
    MSE_pca_list(i)  = mean((X - pca_Yd).^2, 'all');
    SURE_pca_list(i) = mean(SURE_temp_list,'all');
    PSNR_pca_list(i) = PSNR(X,pca_Yd,1.0);
